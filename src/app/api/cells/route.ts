...
export async function GET() {
    ...
    const allGoods = await getAllGoods(response);
    const responseData = await getData(allGoods);
...
async function getAllGoods(response: Response) {
    const result = { rows: [] };
    const first = await response.json();
    let nextUrl = first.meta.nextHref;
